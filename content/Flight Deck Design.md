...
>
> These switches latch into place and have two fixed mechanical positions: ON and OFF.
>
> - **ON** — When pushed in and flush with the panel, the switch is ON. A mechanical shutter opens in the upper half, revealing an illuminated legend (e.g., ON, AUTO, or a flow bar).
> - **OFF** — When the switch is extended (protruding), it is OFF. The mechanical shutter closes, hiding the legend and leaving the top half blank or showing a line.
> - The lower half of many switches has a status light displaying the current system state (e.g., PRESS, FAIL, INOP, FAULT, MAN, VALVE, or OFF).
>
> **Momentary Action Switches**
>
> These are spring-loaded and mechanically return to the extended (rest) position after being released.
>
> - Used to momentarily activate/deactivate systems or reset system logic.
> - Can be operated by a short push (e.g., resetting master lights and aural alerts) or by a push-and-hold action to keep a specific function active.
> - The switch display also indicates the current system status.
